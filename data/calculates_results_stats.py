...
def calculates_results_stats(results_dic):
    ...
    # Loop through the `results_dic`
    for key in results_dic:
        # Check if the image is a dog
        if results_dic[key][3] == 1:
                n_dogs_img = n_dogs_img + 1

                # Check if the dog is correctly classified
                if results_dic[key][1] == results_dic[key][0]:
                        n_match = n_match + 1
                        n_correct_dogs = n_correct_dogs + 1
                        n_correct_breed = n_correct_breed + 1
        else:
                n_notdogs_img = n_notdogs_img + 1
                # Compare to check if the dog is correctly classified
                if results_dic[key][1] != results_dic[key][0]:
                        n_match = n_match + 1
